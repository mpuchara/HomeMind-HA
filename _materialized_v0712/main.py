...
TRAINING_REVISION = "unit-only-all-context-v16"
...
DEFAULT_OPTIONS = {
    ...
    "agent_training_chunk_hours": 48,
    "agent_training_overlap_hours": 12,
    ...
    "history_background_pause_ms": 250,
...
class HistoryManager(threading.Thread):
    ...
    def _eligible_rebuild_context(self):
        ...
        excluded_control, _ = controllable_context_exclusions(current, registry)
    ...
    def set_status(self, phase=None, progress=None, message=None, chunk_done=None, chunk_total=None, stage_eta_seconds=None):
        now = now_ts()
        with self.lock:
            ...
            if progress is not None:
                ...
                if self.cycle_started_at is not None:
                    ...
                    # Keep about ten minutes of samples. ETA is deliberately labelled
                    # an estimate because Recorder chunks can vary greatly in cost.
                    cutoff = now - 600
                    self._progress_samples = [x for x in self._progress_samples if x[0] >= cutoff][-40:]
                    base = None
                    for sample in self._progress_samples:
                        if now - sample[0] >= 8 and p - sample[1] >= 0.005:
                            base = sample
                            break
                    if base is not None:
                        ...
                        # Cap absurd early estimates and smooth the display.
                        eta = clamp(eta, 0.0, 24 * 3600.0)
                        self.eta_seconds = eta if self.eta_seconds is None else (0.65 * self.eta_seconds + 0.35 * eta)
...
class Engine(threading.Thread):
    ...
    def __init__(self):
        ...
        self.ws_error = None
        self.temporal_history = TemporalHistory(maxlen=64)
        self.lock = threading.RLock()
    ...
    def on_state_changed(self, data):
        ...
        if new_state is not None:
            ...
            self.temporal_history.add(entity_id, ts, new_state)
    ...
    def refresh_states(self):
        ...
        for st in state_map.values():
            ...
            self.temporal_history.add(st.get("entity_id"), ts, st)
