...
class Release038PiDegradationTests(unittest.TestCase):
    ...
    def test_frontend_has_one_owner_for_full_agent_and_candidate_reads(self):
        ...
        self.assertIn("lastDiscoveryAgentRefresh", app)
        self.assertIn("refreshAfterDiscovery", app)
        self.assertIn("discovery_revision=", app)
        self.assertIn("encodeURIComponent(completedDiscoveryRun)", app)
        polling = (ROOT / "adaptive_ai/src/static/polling_guard.js").read_text(encoding="utf-8")
        self.assertIn("return path+(u.search||'');", polling)
        self.assertIn("Detected agents will stay idle until you choose which ones to Train.", app)
        self.assertIn("discovery_reason_counts", app)
        self.assertIn("Discovery window:", app)
        self.assertIn("discovery_deep_history_complete", app)
