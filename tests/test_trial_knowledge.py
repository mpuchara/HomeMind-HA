...
class TrialPolicy(DummyPolicy):
    def update(self, horizon, action_idx, features, reward, sample_ts=None):
        ...
        rows.append({"horizon": int(horizon), "action_index": int(action_idx),
                     "reward": float(reward), "features": {str(k): float(v) for k, v in features.items()}})
...
class TrialKnowledgeTests(unittest.TestCase):
    def setUp(self):
        ...
        self.root_model = {
            ...
            "selection_meta": {"schema_revision": 4}, "weights": {"root": 1}, "trial_reward_sum": 0.0,
        }
        ...
        self.store.set_training_state(
            ...
            detail={"balanced": True, "class_coverage": True,
        ...
        self.engine = SimpleNamespace(
            ...
            own_command_echo=lambda *args, **kwargs: False, process_agent=lambda *args, **kwargs: None,
    ...
    def model(self, aid): return self.store.get_model(aid)
    ...
    def trial(self, ack=True):
        ...
        return {
            "trial_id": str(uuid.uuid4()), "kind": "probe", "arm": 1, "value": 1.0,
            "baseline": 0.0, "index": 1, "x": {"bias": 1.0},
            "prediction_inputs": {"binary_sensor.presence": 1.0}, "background_dependencies": {},
            ...
            "trial_record": {
                ...
                "action_set": [{"role": "reference", "index": 0, "value": 0.0, "propensity": .25},
    ...
    def test_positive_trial_updates_only_direct_child_and_not_live_residual(self):
        live = copy.deepcopy(self.model(self.root["id"]))
        gid, child, trial = self.finish_and_train(.6)
        model = self.model(child["agent_id"])
        self.assertAlmostEqual(model["trial_reward_sum"], .6)
        self.assertAlmostEqual(model["trial_reward_by_action"]["1"], .6)
        self.assertEqual(self.model(self.root["id"]), live)
        self.assertEqual(self.engine.experiments._get(self.root["id"])["learners"], {})
        self.assertEqual(self.manager.trial_journal.get(trial["trial_id"])["learning_applied_generation_id"], gid)
    ...
    def test_negative_trial_remains_negative_and_history_is_not_replayed(self):
        ...
        model = self.model(child["agent_id"])
        self.assertAlmostEqual(model["trial_reward_sum"], -1.0)
        self.assertAlmostEqual(model["trial_reward_by_action"]["1"], -1.0)
        self.assertEqual(self.model(self.root["id"]), live)
        self.assertEqual(json.loads(self.manager.trial_journal.get(trial["trial_id"])["episode_result_json"])["reward"], -1.0)
    ...
    def test_restart_retry_does_not_apply_same_trial_twice(self):
        ...
        self.assertEqual(self.model(child["agent_id"]), first)
        self.assertEqual(len(first.get("trial_updates") or []), 1)
    ...
    def test_unlabelled_trial_does_not_update_policy(self):
        ...
        rec = self.manager.trial_journal.get(trial["trial_id"])
        self.assertIsNone(rec["reward"])
        self.assertIsNone(rec["learning_applied_generation_id"])
    ...
    def test_information_exploration_needs_no_positive_base_gradient(self):
        ...
        arms = [dict(chosen, uncertainty=.05),
    ...
    def test_trial_record_has_versioned_assignment_dispatch_ack_and_outcome(self):
        ...
        self.assertEqual(json.loads(rec["hypothesis_json"])["id"], "earlier_on")
        self.assertTrue(json.loads(rec["context_json"])["policy_features"])
        self.assertEqual(json.loads(rec["model_versions_json"])["policy_version"], 10)
        self.assertEqual(len(json.loads(rec["action_set_json"])), 2)
        self.assertAlmostEqual(rec["propensity"], .75)
        self.assertEqual(json.loads(rec["dispatch_json"])["intent_id"], "intent-1")
        self.assertEqual(json.loads(rec["ack_json"])["status"], "acknowledged")
        self.assertEqual(json.loads(rec["episode_result_json"])["reward"], .6)
        self.assertEqual(rec["termination_reason"], "confirmed")
