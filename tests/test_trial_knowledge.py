...
from trial_knowledge import TrialJournal, install as install_trial_knowledge


class TrialPolicy(DummyPolicy):
    def update(self, horizon, action_idx, features, reward, sample_ts=None):
        updates = list(self.model.get("trial_updates") or [])
        updates.append({
            "horizon": int(horizon), "action_index": int(action_idx), "reward": float(reward),
            "features": {str(k): float(v) for k, v in features.items()}, "sample_ts": sample_ts,
        })
        self.model["trial_updates"] = updates
        self.model["trial_reward_sum"] = float(self.model.get("trial_reward_sum") or 0.0) + float(reward)
        by_action = dict(self.model.get("trial_reward_by_action") or {})
        key = str(int(action_idx))
        by_action[key] = float(by_action.get(key) or 0.0) + float(reward)
...
class TrialKnowledgeTests(unittest.TestCase):
    def setUp(self):
        ...
        self.store = storage.Store(Path(self.temp.name) / "trial-knowledge.db")
    ...
    def _start_free(self, *, previous_enabled=False, information=True):
        self.engine.experiments.configure(self.root, {"enabled": previous_enabled})
        return self.manager.workflow_explore(self.root["id"], {
            "mode": "free", "information_exploration": information,
            "config": {"focus": "presence", "intensity": .2, "interval": 1,
                       "daily_budget": 1, "observation_seconds": 5, "max_step": 1},
        })

    def _trial(self, reward_ready=True):
        now = time.time()
        ...
        return {
            ...
            "background_dependencies": {}, "outcome_sources": {"binary_sensor.presence": {"role": "presence", "before": -1.0}},
            ...
            "observation_start": now, "observation_end": now + 5,
            "ack": now if reward_ready else None, "window": 5,
            "trial_record": {
    ...
    def _finish_and_train(self, reward):
        started = self._start_free(previous_enabled=False)
    ...
    def test_positive_trial_updates_only_direct_child_and_not_live_residual(self):
        ...
        child_gid, child, trial = self._finish_and_train(.6)
        ...
        self.assertEqual(record["status"], "labelled")
        self.executor.submit.assert_not_called()
        self.executor.service.assert_not_called()

    def test_negative_trial_remains_negative_for_candidate_and_is_not_a_demonstration(self):
        live_before = copy.deepcopy(self.model(self.root["id"]))
        child_gid, child, trial = self._finish_and_train(-1.0)
        child_model = self.model(child["agent_id"])
        ...
        self.assertFalse((json.loads(record["context_json"]) or {}).get("ordinary_history_replayed", False))
    ...
    def test_restart_retry_does_not_apply_same_trial_twice(self):
        _gid, child, _trial = self._finish_and_train(.6)
        first = copy.deepcopy(self.model(child["agent_id"]))
        row = self.manager._candidate_row(self.root["id"])
        # Simulate worker retry after a process restart: durable application marker wins.
        self.assertTrue(self.manager._start_build(row))
        second = self.model(child["agent_id"])
        self.assertEqual(second, first)
        self.assertEqual(len(second.get("trial_updates") or []), 1)

    def test_unlabelled_trial_never_updates_reward_or_candidate_policy(self):
        started = self._start_free(previous_enabled=False)
        child = self.manager.lineage_status(started["child_generation_id"])
        before_child = copy.deepcopy(self.model(child["agent_id"]))
        trial = self._trial(reward_ready=False)
        self.engine.experiments._start(self.root["id"], trial)
        self.engine.experiments._finish(self.root["id"], None, "no device acknowledgement")
        session = self.manager.workflow_explore_status(self.root["id"])["session"]
        self.assertEqual(session["status"], "no_evidence")
        self.assertEqual(session["measured_outcomes"], 0)
        record = self.manager.trial_journal.get(trial["trial_id"])
        self.assertIsNone(record["reward"])
        self.assertIsNone(record["learning_applied_generation_id"])
        self.assertEqual(self.model(child["agent_id"]), before_child)

    def test_previous_experiment_config_is_restored_after_session(self):
        self.engine.experiments.configure(self.root, {"enabled": False, "focus": "devices", "daily_budget": 4})
        ...
        self.assertEqual(started["session"]["knowledge_integration"], "explicit_child_training_from_trial_records")
    ...
    def test_off_policy_report_refuses_action_without_logged_propensity_coverage(self):
        result = self._start_free(previous_enabled=False)
        ...
        trial["kind"] = "reference"
        trial["index"] = 0
        trial["value"] = 0.0
        self.engine.experiments._start(self.root["id"], trial)
        report = self.manager.trial_journal.off_policy_report(session_id, 1.0)
        self.assertFalse(report["supported"])
        self.assertEqual(report["reason"], "no_propensity_coverage")
        self.assertIsNone(report["estimate"])

    def test_information_exploration_can_probe_without_positive_base_gradient(self):
        result = self._start_free(previous_enabled=False, information=True)
        self.engine.experiments.rng = FixedRng(.9)  # choose the probe branch, not reference
        policy = TrialPolicy(self.store, self.root)
        # All b/a gradients are exactly zero: legacy Free Explore has no candidate.
        chosen = {"index": 0, "value": 0.0, "mean": 0.0, "support": .95, "novelty": .05}
        arms = [
            dict(chosen, uncertainty=.05),
            {"index": 1, "value": 1.0, "mean": 0.0, "support": .95, "novelty": .05, "uncertainty": .8},
        ]
        labels = {0: ["bias"], 1: ["binary_sensor.presence:value"]}
        features = {0: 1.0, 1: 1.0}
        prepared = self.engine.experiments.propose(
            self.root, policy, dict(self.engine.state_map), {}, features, labels,
            chosen, .95, arms, 1.0, {},
        )
        ...
        self.assertEqual(meta["selection_reason"], "information_exploration")
        self.assertAlmostEqual(meta["assigned_propensity"], .75)
        self.assertIsNone(prepared["gain"])
        self.assertEqual(result["session"]["hypothesis_catalog"], ["earlier_on"])
        self.executor.submit.assert_not_called()
        self.executor.service.assert_not_called()

    def test_trial_record_contains_required_versioned_assignment_and_outcome_fields(self):
        self._start_free(previous_enabled=False)
        trial = self._trial()
        self.engine.experiments._start(self.root["id"], trial)
        self.manager.trial_journal.dispatch(trial["trial_id"], time.time(), intent_id="intent-1", desired_value=1.0)
        ...
        self.assertTrue(json.loads(record["outcome_sources_json"]))
