"""Fresh-process tests of run.sh's actual entrypoint, with no HA/network workers."""
import os
import subprocess
import sys
import tempfile
...
class PackagedStartupTests(unittest.TestCase):
    ...
    def test_import_entrypoint_does_not_touch_database(self):
        self.run_isolated('''
import sys
import fast_queue_main as entry
...
assert 'manual_context_learning' not in sys.modules
''')
...
with patch.object(http.server, 'ThreadingHTTPServer', side_effect=bind), \
    ...
    runpy.run_path('adaptive_ai/src/fast_queue_main.py', run_name='__main__')
...
''')
    ...
    def test_http_remains_responsive_while_extensions_initialize(self):
        self.run_isolated('''
import json
import threading
