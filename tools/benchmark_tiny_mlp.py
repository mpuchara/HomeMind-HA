#!/usr/bin/env python3
"""Stage-3 benchmark for the real inference-only TinyMLPBackend.

Runs entirely on synthetic observations.  It never connects to Home Assistant, never
creates ActionIntent and never exercises Executor.  Results from CI are not Raspberry Pi
measurements; the script is packaged so the same probe can be run on target hardware.
"""
...
def rss_bytes():
    # Linux current RSS is preferable to ru_maxrss because the 1/20/50 model scenarios
    # are intended to show actual resident growth, not only a process lifetime maximum.
    try:
...
def benchmark(iterations=200, feature_count=96, outputs=2):
    ...
    for count in (1, 20, 50):
        ...
        # Exercise every loaded model repeatedly without making the 50-model CI probe
        # dominate the full suite.
        repeated = []
    ...
    # Keep the final scenario alive until all RSS samples have been taken.
    assert retained
    inference_summary = {
