...
function renderAgents(){
  ...
  $('#agents').innerHTML=agents.length?agents.map(a=>{
    ...
    return `<article class="agent card">
      ...
      <div class="agent-primary"><div class="state-metric current"><span>Current</span><b>${currentValue(a)}</b></div><div class="state-metric desired"><span>Desired</span><b>${prediction(a)}</b></div><div><span>Calibrated confidence</span><b>${pct(conf)}</b></div><div><span>Support</span><b>${pct(support)}</b></div><div><span>Novelty</span><b>${pct(novelty)}</b></div><div><span>History</span><b>${num(a.historical_count||0)}</b></div></div>
      ...
      <div class="agent-status"><span>${rt.realtime_connected?'⚡ realtime':'REST fallback'}</span><span>reacts ~${rt.prediction_horizon||0}s ahead</span><span>${ctx.selected_entities||0}/${ctx.considered_entities||ctx.whole_home_entities||0} context selected</span><span>${ctx.dimensions||128} explicit dims</span></div>
