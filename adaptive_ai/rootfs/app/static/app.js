...
function renderAgents(){
  ...
  $('#agents').innerHTML=agents.length?agents.map(a=>{
    ...
    return `<article class="agent card">
      ...
      <div class="agent-primary"><div class="state-metric current"><span>Current</span><b>${currentValue(a)}</b></div><div class="state-metric desired"><span>Desired</span><b>${prediction(a)}</b></div><div><span>Confidence</span><b>${pct(conf)}</b></div><div><span>Validation</span><b>${pct(rt.validation_accuracy||0)}</b><small>${num(rt.validation_samples||0)} samples</small></div><div><span>Support</span><b>${pct(support)}</b></div><div><span>Novelty</span><b>${pct(novelty)}</b></div></div>
      ...
      <div class="agent-status"><span>${rt.realtime_connected?'⚠ realtime':'REST fallback'}</span><span>reacts ~${rt.reaction_latency_ms||75} ms</span><span>manual lead target ~${rt.manual_lead_target_seconds||1}s</span><span>${ctx.selected_entities||0}/${ctx.considered_entities||ctx.whole_home_entities||0} context selected</span><span>${ctx.dimensions||128} explicit dims</span></div>
      ...
      <details class="agent-details" data-agent-id="${esc(a.id)}" ${openAgentDetails.has(String(a.id))?'open':''}><summary><span>Control diagnostics & learning</span><span class="details-chevron" aria-hidden="true">⌄</span></summary>
        <div class="detail"><b>Last HA service:</b> ${service}${rt.last_service_error?` · ${esc(rt.last_service_error)}`:''}<br><b>Model:</b> ${esc(rt.model||'—')}<br><b>Reaction:</b> ~${rt.reaction_latency_ms||75} ms after a context event · target ~${rt.manual_lead_target_seconds||1}s before manual action<br><b>Validation:</b> ${pct(rt.validation_accuracy||0)} raw hit rate · conservative lower bound ${pct(rt.validation_lower_bound||0)} · n=${num(rt.validation_samples||0)}<br><b>Historical support / novelty:</b> ${pct(support)} / ${pct(novelty)}<br><b>History rewards:</b> ${num(a.historical_count||0)}<br><b>Live reward:</b> ${rt.last_reward==null?'—':Number(rt.last_reward).toFixed(2)} ${rt.last_reward_reason?' · '+esc(rt.last_reward_reason):''}${rt.pending_feedback?' · awaiting feedback':''}<br><b>Automation hints:</b> ${ctx.automation_hint_entities||0} upstream entities</div>
