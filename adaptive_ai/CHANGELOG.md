...
- Measured scale result: 4661 possible semantic features → 65 selected; selector materialization 13.81 ms p50 / 15.32 ms p95 outside the hot path; Ridge p95 106.81 µs before vs 105.27 µs after mask materialization (ratio 0.986), with zero selector calls and exact decision parity in the measured event-path loop.
