...
function noAgentsMessage(h){if(h?.phase==='ready')return 'No active controllable target met the discovery criteria. Try Rescan devices; config/diagnostic entities are intentionally ignored.';return 'Discovery is running. Agents appear in WAITING and train only when requested.';}
...
function renderAgents(){
  ...
  $('#agents').innerHTML=agents.length?agents.map(a=>{
    ...
    return `<article class="agent card ${paused?'paused-agent':''} ${indexing?'training-agent':''}">
      ...
      <div class="actions">${['shadow','control','paused'].map(m=>`<button class="ghost ${a.mode===m?'active':''}" ${(m==='control'&&!qualified)||indexing?'disabled title="Requires completed benchmark >78%"':''} onclick="setMode('${a.id}','${m}')">${m}</button>`).join('')}${neverTrained?`<button class="ghost resume" onclick="trainAgent('${a.id}')">Train</button>`:(paused?`<button class="ghost resume" onclick="resumeLearning('${a.id}')">Resume</button>`:'')}<button class="ghost" onclick="editAgent('${a.id}')">Ustawienia</button><button class="ghost" ${a.mode!=='control'||!qualified?'disabled':''} onclick="verifyControl('${a.id}')">Verify control</button><button class="ghost ${a.micro_exploration?'active warn':''}" disabled title="0.9 uses deterministic policies; exploration remains a stored preference for a future backend" onclick="toggleExplore('${a.id}',${a.micro_exploration?'false':'true'})">Deterministic</button><button class="ghost rebuild" ${indexing||neverTrained?'disabled':''} onclick="resetLearning('${a.id}')">Rebuild</button><button class="ghost danger" onclick="removeAgent('${a.id}')">Delete</button></div>
    </article>`;
