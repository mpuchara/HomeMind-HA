...
function automaticCorrectDiagnostics(rt){
  ...
  const confidence=last.confidence==null?'—':pct(Number(last.confidence));
  const reliability=last.source_reliability==null?'—':pct(Number(last.source_reliability));
  const source=last.source_entity_id||last.source_origin||'—';
  const trial=last.trial_id||last.decision_id||'—';
  const actionAt=last.action_ts?new Date(Number(last.action_ts)*1000).toLocaleString():'—';
...
function observationSelection(a){
  ...
  if(!o.mask_id)return `<div class="context-list"><b>Observation space:</b> ${esc(o.status||'not materialized')}</div>`;
  const rows=(o.features||[]).slice(0,12);
...
async function trainAgent(id){
  ...
  // Queue admission succeeded. A later renderer/refresh problem must never be reported
  // as a failed Train request, because that encourages duplicate clicks/jobs.
  try{await load();}catch(e){
    console.error('Training was accepted; UI refresh will retry automatically',e);
  }
}
