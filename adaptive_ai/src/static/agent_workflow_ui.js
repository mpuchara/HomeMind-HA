...
(()=>{
  ...
  window.workflowAutonomous=async(ref,button)=>{
    ...
    try{
      ...
      const out=await post(ref,'autonomous');await refresh();return out;
  ...
  window.workflowChangeDecision=async(ref,button)=>{
    if(button)button.disabled=true;
    try{
      const s=await status(ref);let desired;
      if(s.target_property!=='power'){
        const raw=prompt(`Nowe Desired dla ${s.name} (${s.min_value}–${s.max_value}):`,s.observed_desired??s.current??'');
        if(raw===null)return;desired=Number(String(raw).replace(',','.'));if(!Number.isFinite(desired))throw Error('Podaj poprawną liczbę');
      }
      const out=await post(ref,'change-decision',desired===undefined?{}:{desired_value:desired});await refresh();return out;
    }catch(e){notifyError(e);}finally{if(button)button.disabled=false;}
  };
  ...
  const dialog=document.createElement('dialog');dialog.id='correctDialog';document.body.append(dialog);
  ...
  function shell(){
    dialog.innerHTML=`<div class="teach-head"><h2>Correct: ${html(subject.name)} · Gen ${subject.generation_number}</h2><button class="ghost" data-close>Zamknij</button></div>
      ...
      <p class="teach-legend" data-legend></p><div class="teach-chart" data-chart></div><p data-status role="status"></p><p data-error role="alert"></p>
    ...
    dialog.querySelector('[data-undo]').onclick=undo;dialog.querySelector('[data-apply]').onclick=apply;dialog.querySelector('[data-point]').onsubmit=save;
  }

  window.openWorkflowCorrect=async generationRef=>{try{ref=String(generationRef);subject=await status(ref);const end=Date.now()/1000;range={start:end-600,end};selected=null;data=null;shell();dialog.showModal();await load();}catch(e){notifyError(e);}};
  ...
  function renderLegend(){
    const legend=dialog.querySelector('[data-legend]');if(!legend||!data)return;const series=data.series||{},rows=[];
    ...
    rows.push(['Correct points',COLORS.correct,false]);legend.innerHTML=rows.map(([label,color,dashed])=>`<span style="color:${color}">${dashed?'┄':'●'} ${html(label)}</span>`).join('');
  }
  ...
  async function load(){
    ...
    try{
      const out=await api(`api/agent-workflow/${encodeURIComponent(ref)}/correct-history?start=${range.start}&end=${end}`);if(seq!==requestSeq||!dialog.open)return;
      data=out;renderLegend();draw();const current=(out.series?.current?.points||[]).length,labels=(out.labels||[]).length;const childGaps=(out.gaps||[]).length,parentGaps=(out.parent_gaps||[]).length,gaps=childGaps+parentGaps;
      dialog.querySelector('[data-status]').textContent=`${current} obserwacji · ${labels} Correct points${gaps?` · ${gaps} luk runtime`:''}. Direct parent comparison; policy replay wyłączony.`;dialog.querySelector('[data-undo]').disabled=!labels;
    }catch(e){if(seq===requestSeq)error(e);}
  }

  async function inspect(ts){
    if(!Number.isFinite(ts)){error(Error('Wybierz poprawny moment'));return;}selected=null;dialog.querySelector('[data-save]').disabled=true;dialog.querySelector('[data-error]').textContent='';
    try{
      const p=await api(`api/agent-workflow/${encodeURIComponent(ref)}/correct-point?ts=${ts}`);selected=p;dialog.querySelector('[data-time]').value=local(p.ts);const info=dialog.querySelector('[data-point-info]');
      ...
      if(p.generation_type==='candidate')info.textContent=`Current: ${fmt(subject,p.current)} · ${p.parent_desired_label||'Parent Desired'}: ${fmt(subject,p.parent_desired)} · ${p.candidate_desired_label||'Candidate Desired'}: ${fmt(subject,p.candidate_desired)}${p.confidence==null?'':` · Candidate confidence ${(Number(p.confidence)*100).toFixed(1)}%`}${p.context_complete?'':' · niepełny kontekst'}`;
      else info.textContent=`Current: ${fmt(subject,p.current)} · Live Desired: ${fmt(subject,p.live_desired)}${p.confidence==null?'':` · Confidence ${(Number(p.confidence)*100).toFixed(1)}%`}${p.context_complete?'':' · niepełny kontekst'}`;
      const input=dialog.querySelector('[data-value]');input.min=subject.min_value;input.max=subject.max_value;input.value=subject.target_property==='power'?(Number(p.desired)>=.5?0:1):p.desired;dialog.querySelector('[data-save]').disabled=!(p.current!=null&&p.context_complete&&p.desired!=null);draw();
    }catch(e){error(e);}
  }

  async function save(ev){ev.preventDefault();if(!selected)return;const desired=Number(dialog.querySelector('[data-value]').value);if(!Number.isFinite(desired)){error(Error('Podaj poprawne Desired'));return;}setBusy(true);try{await post(ref,'correct-label',{sample_ts:selected.ts,desired_value:desired});selected=null;await load();}catch(e){error(e);}finally{setBusy(false);}}
  ...
  async function apply(){setBusy(true);dialog.querySelector('[data-error]').textContent='';try{const out=await post(ref,'correct');dialog.close();await refresh();return out;}catch(e){error(e);}finally{setBusy(false);}}

  function draw(){
    const box=dialog.querySelector('[data-chart]');if(!box||!data)return;const series=data.series||{},allSeries=[];
    ...
    const lo=Math.min(Number(subject.min_value),...(values.length?values:[Number(subject.min_value)])),hi=Math.max(Number(subject.max_value),...(values.length?values:[Number(subject.max_value)]));
    ...
    const path=(points,{breakOnStale=true}={})=>{let d='',active=false,last=0;for(const p of (points||[]).slice().sort((a,b)=>Number(a.ts)-Number(b.ts))){const v=p.value,ts=Number(p.ts);if(v==null||!Number.isFinite(Number(v))||(breakOnStale&&last&&ts-last>stale))active=false;if(v!=null&&Number.isFinite(Number(v))){d+=active?` H${x(ts)} V${y(v)}`:` M${x(ts)},${y(v)}`;active=true;last=ts;}}return d;};
    const rendered=[];
    if(series.live_desired)rendered.push(`<path data-series="live_desired" d="${path(series.live_desired.points)}" fill="none" stroke="${COLORS.candidate}" stroke-width="2" stroke-dasharray="8 6" stroke-linecap="round"/>`);
    if(series.parent_desired)rendered.push(`<path data-series="parent_desired" d="${path(series.parent_desired.points)}" fill="none" stroke="${COLORS.parent}" stroke-width="2" stroke-dasharray="8 6" stroke-linecap="round"/>`);
    if(series.candidate_desired)rendered.push(`<path data-series="candidate_desired" d="${path(series.candidate_desired.points)}" fill="none" stroke="${COLORS.candidate}" stroke-width="2" stroke-dasharray="8 6" stroke-linecap="round"/>`);
    if(series.current){const currentPath=path(series.current.points,{breakOnStale:false});rendered.push(`<path data-series="current-outline" d="${currentPath}" fill="none" stroke="#04111f" stroke-width="6" stroke-linejoin="round" stroke-linecap="round" opacity="0.92"/>`);rendered.push(`<path data-series="current" d="${currentPath}" fill="none" stroke="${COLORS.current}" stroke-width="3" stroke-linejoin="round" stroke-linecap="round"/>`);}
    ...
    const fraction=ev=>{const r=svg.getBoundingClientRect(),px=(ev.clientX-r.left)*1030/r.width;return Math.max(0,Math.min(1,(px-50)/930));};const hideSelection=()=>selection.setAttribute('visibility','hidden');
    ...
    svg.onwheel=ev=>{ev.preventDefault();zoom(ev.deltaY<0?.5:2,fraction(ev));};hit.onpointerdown=ev=>{down=fraction(ev);showSelection(down);hit.setPointerCapture(ev.pointerId);};hit.onpointermove=ev=>{if(down!=null)showSelection(fraction(ev));};hit.onpointercancel=()=>{down=null;hideSelection();};hit.onlostpointercapture=()=>{if(down!=null){down=null;hideSelection();}};
  ...
  function liveActions(card,a){const actions=card.querySelector('.actions');if(!actions)return;if(actions.dataset.generationWorkflow==='1')return;actions.dataset.generationWorkflow='1';actions.innerHTML=`<button class="ghost" data-wf="auto">Autonomous</button><button class="primary" data-wf="correct">Correct</button><button class="ghost" data-wf="explore" disabled title="Explore będzie wdrożone w następnym PR">Explore</button><button class="ghost" data-wf="change">Change decision</button><button class="ghost" data-wf="settings">Settings</button>`;actions.querySelector('[data-wf=auto]').onclick=e=>workflowAutonomous(a.id,e.currentTarget);actions.querySelector('[data-wf=correct]').onclick=()=>openWorkflowCorrect(a.id);actions.querySelector('[data-wf=change]').onclick=e=>workflowChangeDecision(a.id,e.currentTarget);actions.querySelector('[data-wf=settings]').onclick=()=>workflowSettings(a.id);}

  const baseRender=window.renderAgents;
  if(typeof baseRender==='function'&&!window.__generationWorkflowRender){window.renderAgents=()=>{const out=baseRender(),agents=liveAgents();document.querySelectorAll('#agents > .agent:not(.candidate-agent)').forEach(card=>{const a=agents.find(x=>String(x.id)===String(card.dataset.agentId));if(a)liveActions(card,a);});return out;};window.__generationWorkflowRender=true;}
